...
//Customer purchase of three of each item at Warehouse 3.  This setup will only work for one warehouse at a time
System.out.println("\nCustomer Purchase:");
System.out.println("\nQuantity");
...
//This information would need to be updated manually to reflect the appropriate warehouse
...
//Output for shipment in.  This will only work for one item at a time, but can be scaled up for multiple warehouses, provided the shipment is the same size.
    System.out.println("\nShipment Details:");
...
//Again, this would have to be manually updated for any changes, so is not a good general approach, but could be used to track simple changes.  Should probably use a for-loop here of some kind, but ran out of time to be able to install one.
